...
export default class FacebookDataTable extends Component {
  ...
  createHeader(cellInfo) {
    ...
    if (hideColumns) {
      const handleColumnHide = (id) => {
        let newColumnOrders = this.state.columnOrders.filter((column) => column !== cellInfo.col);
        this.setState({ columnOrders: newColumnOrders });
      }
      headerCell=<helperCells.RemovableHeaderCell callback={handleColumnHide}
      >
        {cellInfo.header}
      ...
      headerCell = <helperCells.SortHeaderCell
  ...
  columnGroup(dataList, columnGroupName, index) {
    ...
    return (<ColumnGroup
      key={index}
      header={<Cell>{columnGroupName}</Cell>}
    >
  ...
  cellViews(collumnName, dataList) {
    ...
    const cellInfo = this.props.tableInfo.cells.filter((cell) => cell.col === collumnName)[0];
    const width = columnWidths[cellInfo.col] ? columnWidths[cellInfo.col] : cellInfo.width;
    let ComponentCell;
    switch(cellInfo.coulumnType) {
      case 'data': ComponentCell = helperCells.DateCell; break;
      case 'image': ComponentCell = helperCells.ImageCell; break;
      case 'link': ComponentCell = helperCells.LinkCell; break;
      default: ComponentCell = helperCells.TextCell;
    }
  ...
  reOrderColumns(event) {
    const columnOrders = this.state.columnOrders.filter((columnKey) => columnKey !== event.reorderColumn);
  ...
  filterDataList() {
    ...
    if (Object.keys(colSortDirs).length === 1) {
      ...
      const filteredIndexes = this.filterSearchText(allColumns[1], searchText, dataList.datas);
  ...
  render() {
    ...
    return(<div>
      {tableInfo.hideColumns ? <Button type="primary" onClick={this.resetColumns}>Reset</Button>: '' }
      {tableInfo.searchText ? <SearchInput
      ...
      <Table
        ...
        onColumnResizeEndCallback={tableInfo.resizable ? this.resizeColumns : undefined}
        onColumnReorderEndCallback={tableInfo.reorderable ? this.reOrderColumns: undefined}
        width={1000}
        height={500}

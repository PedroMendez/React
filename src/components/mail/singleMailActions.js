  ...
  render() {
    return (
      <Popconfirm
        ...
        cancelText="No"
        onConfirm={() => {
          notification('error', `Deleted selectd mail`, '');
        }}
      >
...
class MoveMailButton extends Component {
  render() {
    const bucketOptions = buckets.map(bucket => <li
        onClick={() => {
          notification('success', `Massage Moved Successfully`, '');
        }}
        key={bucket}
      >{bucket}</li>);
    const content = <ul>{bucketOptions}</ul>;
    return (
      <Popover
        title={`Move mail`}
        content={content}
...
class SelectTagButton extends Component {
  render() {
    const tagOptions = tags.map(tag => <li
        onClick={() => {
...
export default class MailAction extends Component {
  render() {
    ...
    return (
      <div className="isoMailActionsController">
        {toggleListVisible
          ? <button className="mailBackBtn" onClick={toggleView}>
              Inbox
            </button>
          : ''}
        <div className="isoMailActions">
          <button
            type="button"
        ...
        <div className="isoMailMove">
          <MoveMailButton />
          <SelectTagButton />
        </div>

        <div className="isoSingleMailPagination">
          {index === 0
            ? ''
            : <button
                type="button"
